      ...
      body: SafeArea(
        child: ListView(
          ...
          children: [
            ...
            _HelpTile(
              step: '4.5',
              title: '契約外の時間で出勤する日は「任意の時間で入力」',
              accentColor: AppColors.customTime,
              body:
                  '以前は契約外の時間で出勤する日、メモ欄に「13:00〜18:00」のように'
                  '手打ちで入力して提出していただいていましたが、その必要はなくなりました。\n\n'
                  '「任意の時間で入力」ボタンをタップし、ダイヤルを回して開始・終了時刻を'
                  '選ぶだけで、時間が自動でカレンダーに反映されます。休憩10分の'
                  '有無もその場で選べます。\n\n'
                  'A〜Yの記号を選んだうえで、さらに任意の時間を追加で入力することもできます。',
            ),
            _HelpTile(
              step: '5',
              title: '有給休暇を入力する',
            ...
            _HelpTile(
              ...
              title: '入力が終わったら送信する',
              body:
                  '画面下部の「シフト送信」ボタンをタップすると、「シフトを送信しますか？」という確認画面が表示されます。'
                  '氏名・部署・対象月に間違いがないか確認し、「送信する」をタップすると送信されます。\n\n'
                  '内容を直したいときは「キャンセル」をタップすれば、送信されずに入力画面に戻ります。\n\n'
                  '送信が成功すると「送信完了」というメッセージが表示されます。送信ボタンを押し忘れると、内容は管理者に届きません。必ず最後まで進めてください。',
              warn: true,
            ),
            ...
            _HelpTile(
              step: '💰',
              title: '「自分の給料計算」カードを開く',
              accentColor: AppColors.primaryDeep,
              body:
                  '入力画面の下のほうにある「自分の給料計算（任意）」というカードをタップすると開き、'
                  'もう一度タップすると閉じます。同僚に画面を見られても給料の情報が見えないよう、'
                  '最初は閉じた状態になっています。',
            ),
            _HelpTile(
              step: '💴',
              title: '時給を入力すると自動で計算される',
              accentColor: AppColors.primaryDeep,
              body:
                  'カードを開いて「時給」欄に自分の時給を入力すると、今月入力した勤務時間（有給休暇を含む）'
                  'から「今月の給料（見積り）」が自動で計算されて表示されます。\n\n'
                  '・入力した時給はこの端末にのみ保存され、管理者には送信されません\n'
                  '・有給休暇（有）の日も、時間を入力すれば給料計算に含まれます\n'
                  '・あくまで目安の見積りです。実際の給料は管理者が計算する内容が優先されます',
            ),
            const SizedBox(height: 24),
            const _SectionHeader(
              title: '確定したシフトを確認する',
            ...
            _HelpTile(
              ...
              body:
                  '管理者が自分の部署のシフトを確定して配布すると、画面上部に緑色のバナー'
                  '「シフトが確定しました。月間シフト一覧表を見る」が表示されます。タップすると、'
                  '自分の部署の全員分のシフトが1つの表で確認できます。\n\n'
            ...
            _HelpTile(
              ...
              body:
                  'この画面（シフト希望の入力画面）を開いたままにしていると、管理者が配布した'
                  '瞬間に画面下部に通知（メッセージ）が表示され、バナーも自動的に現れます。'
                  '画面を下に引っ張って更新する必要はありません。\n\n'
                  '※ アプリを閉じている間は通知が届きません。配布されたかどうかは、この画面を'
                  '開いたときにバナーが出ているかどうかで確認してください。',
            ),
            const SizedBox(height: 24),
            const _SectionHeader(
              title: 'バージョン表示・アップデート情報',
              icon: Icons.new_releases_outlined,
            ),
            const SizedBox(height: 8),
            _HelpTile(
              step: 'v',
              title: '画面タイトルの下にある「ver1.3」などの表示',
              body:
                  '入力画面のタイトルのすぐ下に、現在のアプリのバージョン（例：ver1.3）が小さく表示されています。'
                  'タップすると、これまでの更新内容（アップデート履歴）を日付ごとに確認できます。',
            ),
            _HelpTile(
              step: '新',
              title: '赤い「NEW」マークが出ているとき',
              accentColor: AppColors.success,
              badge: const _ColorBadge(
                label: 'NEW',
              ...
              body:
                  '前回この端末で確認したときよりも新しいバージョンが公開されている場合、'
                  'バージョン表示の横に赤い「NEW」マークが表示されます。\n\n'
                  'マークをタップしてアップデート履歴を開くと、新しい更新内容を確認できます。一度開くと'
                  '「既読」として記録され、NEWマークは消えます。',
            ),
            const SizedBox(height: 24),
            const _SectionHeader(
              title: 'よくある質問（Q&A）',
            ...
            const _FaqTile(
              question: '契約外の時間で出勤する日は、今までみたいにメモに書けばいいですか？',
              answer:
                  'いいえ、その必要はありません。日付をタップして「任意の時間で入力」を'
                  '押すと、ダイヤルを回して開始・終了時刻を選べます。選んだ時間は'
                  '自動でカレンダーに反映されるので、メモへの手打ちは不要です。',
            ),
            const _FaqTile(
            ...
            const _FaqTile(
              question: '「月間シフト一覧表を見る」バナーが出てきません。',
              answer:
                  '管理者がまだあなたの部署のシフトを配布していない可能性があります。部署は正しく'
                  '選択されているか確認のうえ、しばらく待ってから確認してください。管理者に配布'
                  '済みか直接確認するのも確実です。',
            ),
            const _FaqTile(
              question: '他の部署のシフト一覧表も見られますか？',
              answer:
                  'いいえ。一覧表で確認できるのは、自分が選択している部署のシフトのみです。'
                  '他の部署のシフトは表示されません。',
            ),
            const _FaqTile(
              question: 'アプリを閉じていても配布の通知は届きますか？',
              answer:
                  '現在は、シフト希望の入力画面を開いている間のみ通知（画面下部のメッセージと'
                  'バナー）が表示される仕組みです。アプリを閉じている間は通知が届きませんので、'
                  '画面を開いたときにバナーが出ているかどうかで配布状況を確認してください。',
            ),
            const _FaqTile(
              question: '給料の見積りは実際の給料と完全に一致しますか？',
              answer:
                  'いいえ、あくまで目安です。入力した時給と、今月希望として入力した勤務時間'
                  '（有給休暇を含む）をもとに計算した見積り額であり、実際の給料計算（残業・'
                  '深夜手当・税金などを含む）は管理者側の計算内容が優先されます。参考の目安として'
                  'ご利用ください。',
            ),
          ],
        ),
